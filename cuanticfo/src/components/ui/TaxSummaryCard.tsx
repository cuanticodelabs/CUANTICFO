  ...
  return (
    <div className={`card p-5 ${className || ''}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-slate-400 bg-slate-50 px-2 py-1 rounded-lg capitalize">
          {data.periodo}
        </span>
      </div>
      ...
      <div className="space-y-2">
        {rows.map((row) => {
          ...
          return (
            ...
            >
              <span
                className={`text-sm ${row.bold ? 'text-slate-800 font-semibold' : 'text-slate-600'}`}
              >
                {row.label}
              </span>
              <span
      ...
      {/* Total */}
      <div className="mt-3 pt-3 border-t border-slate-200 flex justify-between items-center bg-blue-50 -mx-5 -mb-5 px-5 py-3 rounded-b-xl">
        <span className="text-sm font-semibold text-blue-900">Total estimado a pagar</span>
